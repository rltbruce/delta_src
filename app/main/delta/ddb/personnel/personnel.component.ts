...
NouvelItem  : boolean;
    ...
    columns = [
      { name: 'Code', prop: 'code' }, 
      { name: 'nom',   prop: 'nom', sortable: true }, 
   ...
   modifier()
  {
    ...
    this.lib=this.selectedItem.libgrade;
  ...
  {    
    ...
    if (this.NouvelItem==false) 
    {
      ...
      let kk=this.rows_grade;
      kk = kk.filter((obj)=>
        {   
           return obj.id==debours.grade;
        });
        console.log("valeur kk");
        console.log(kk);
        console.log(kk[0].libelle);
      let insert_data = this.serializeData(data);
      
      this.index_api.add('Personnel',insert_data , config).subscribe((response) =>
      {
        ...
        if (this.NouvelItem == false) 
        {
          console.log("manova");
            ...
            if(suppression==0) 
            {
              ...
              this.selectedItem.nom  = debours.nom;
              this.selectedItem.code        = debours.code;
              this.selectedItem.grade   = debours.grade;
              this.selectedItem.telephone     = debours.telephone;
              this.selectedItem.libgrade=kk[0].libelle;              
